...
public class BreadthFirst {
	...
	static public void main( String arg[] ) throws IllegalArgumentException, SecurityException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, JSAPException, IOException {
		...
		pl.itemsName = "nodes";
                //Label[] labelStream; 
                /*FileInputStream is = new FileInputStream("try.labels");
                InputBitStream bitstream = new InputBitStream(is);
                
                 * for (int i = 0; i<50;i++){
                    int ankit = bitstream.readGamma();
                    System.out.println("The read number is "+ankit);
                }*/
                Label labelstream[] = ((ArcLabelledImmutableGraph)graph).labelArray(0);
                int m = graph.outdegree(0);
                for(int i=0; i<m ; i++){
                    System.out.println("The output is "+labelstream[i].toString());
                }
                String A = labelstream[2].toString();
                for(int i=0;i<A.length();i++)
                    System.out.println("The character is "+A.charAt(i));
				...
				while( ! queue.isEmpty() ) {
					...
					while( d-- != 0 ) {
						...
						if ( dist[ succ ] == Integer.MAX_VALUE && dist[ curr ] + 1 <= maxDist ) {
							reachable++;
                                                        //Long L = ((BitStreamArcLabelledImmutableGraph)graph).offset.getLong(curr);
                                                        //System.out.println("The offset for"+curr+"is"+L);
		...
		pl.done();
                /*LazyIntIterator successors, succs2;
		try {
                        //successors = graph.successors(41);
                        succs2 = graph.successors(22);
                }
